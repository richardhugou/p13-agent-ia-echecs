...
L'application est disponible sur le port du front (4200), l'API expose sa documentation Swagger (8000/docs). Premier lancement : prévoir le téléchargement des images et du modèle d'embeddings (~quelques minutes).

> ⚠️ Projet en cours de développement — voir `docs/13-checklist-avancement.md` pour l'état réel.
...
Schémas détaillés (services, graphe LangGraph) : `docs/05-architecture-technique.md`.

## Variables d'environnement (`.env.example`)

| Variable | Rôle | Exemple/défaut |
|---|---|---|
| `LLM_PROVIDER` / `LLM_MODEL` / `LLM_API_KEY` | LLM de synthèse | anthropic / claude-haiku-4-5 |
...
| `EMBEDDING_MODEL` | modèle sentence-transformers | Qwen/Qwen3-Embedding-0.6B |
...
docs/       # conception, décisions, checklist d'avancement, gold set
...
Gitflow simplifié : `main` (stable, démo) ← `develop` (intégration) ← `feature/<nom>`.
