# [LIVRABLE] Étude de faisabilité & coûts — système d'analyse vidéo (build + opex)

> **Comment compléter** : les prix unitaires sont des ordres de grandeur publics ⚠️ à re-vérifier à la date de rédaction (liens à ajouter). La méthode (hypothèses × prix unitaires, 3 scénarios, sensibilité) est le livrable autant que les chiffres : tout doit être recalculable en changeant une hypothèse.
...
| Compute pipeline complet | ~3 min CPU/vidéo (extraction + détection + FEN + alignement) | à re-mesurer sur pilote [MESURE] |
| Transcription | sous-titres YouTube quand disponibles (~70 % ⚠️) ; sinon API type Whisper ≈ 0,006 $/min | poste dominant, voir sensibilité |
| Enrichissement LLM (résumé, alignement sémantique) | ~5k tokens in + 1k out par vidéo (modèle éco ≈ 1 $/M in, 5 $/M out ⚠️) | ≈ 0,01 $/vidéo |
| Périmètre juridique MVP | vidéos **Creative Commons** uniquement + transcripts | lève le risque CGU (cf. note §4.1) |
| TJM développement | 500 € (junior encadré, France) ⚠️ ajustable | pour le build |
...
| **V2** | Plateaux 3D : dataset (annotation si nécessaire : 5–10 k€ ⚠️), fine-tuning vision (GPU : quelques dizaines d'heures de T4, coût GPU **marginal : < 50 €** ⚠️), tracking expériences (MLflow/W&B), déploiement inference | 40–80 j-h | 20–40 k€ (+ data) |
...
Indexation humaine : 15–30 min/vidéo × ~30 €/h chargé ≈ **7,5–15 €/vidéo**, contre **~0,10–0,15 €/vidéo** automatisé (S2) → facteur **×50 à ×100**, hors qualité/exhaustivité. (Et l'humain reste dans la boucle en contrôle qualité par échantillonnage : à chiffrer ~[X] h/mois.)
...
| Plateaux 3D filmés | Modèles publics + datasets (ChessReD ⚠️) mais domain gap réel | **Risqué — V2 seulement, après pilote** |
...
- **Critères go/no-go après pilote MVP (100 vidéos)** : précision FEN ≥ [90] % sur échantillon annoté ; coût/vidéo ≤ [0,20] € ; ≥ [30] % des recommandations de l'agent enrichies d'un timestamp ; validation juridique du sourcing.
